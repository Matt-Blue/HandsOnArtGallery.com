...
{    
    ...
    public function Gallery($category){
        ...
        if(isset($category) && $category !== 'all'){
            $artworks = DB::table('gallery')->where('category', '=', $category)->get();
        }else{
            $artworks = DB::table('gallery')->get();
        }
        // edit mode for admin only
        if(Auth::user() && Auth::user()->email === \Config::get('constants.super_admin')){
            return view('gallery')->with('artworks', $artworks)->with('update', true)->with('category', $category);
        }
        return view('gallery')->with('artworks', $artworks)->with('update', false)->with('category', $category);
    }

    public function CreateView(){
        return view('gallery/artwork')->with('update', false);
    }
